...
export class WordPresenter extends BaseComponent implements ISubscriber {
  ...
  update(pub: IPublisherWordList): void {
    ...
    this.playlist.push(word.audio, word.audioExample, word.audioMeaning);
  ...
  private switchButtonAudioPlay(btn: HTMLButtonElement): void {
    btn.innerHTML = this.getPlayIcon(!this.isPlaySound);
    this.isPlaySound = !this.isPlaySound;
    this.PlayArray(this.playlist);
  }
  ...
  private getButtonAudioPlay(): HTMLButtonElement {
    ...
    buttonAudio.innerHTML = this.getPlayIcon(this.isPlaySound);
    buttonAudio.addEventListener('click', () => {
      this.switchButtonAudioPlay(buttonAudio);
    });
    return buttonAudio;
  }

  private PlayArray(urls: string[]) {
    const player = new Audio();
    const iteratorUrls = urls[Symbol.iterator]();
    function setTrack() {
      const currentTrack = iteratorUrls.next().value;
      if (currentTrack) {
        console.log(`${BASE_URL}/${currentTrack}`);
        player.src = `${BASE_URL}/${currentTrack}`;
